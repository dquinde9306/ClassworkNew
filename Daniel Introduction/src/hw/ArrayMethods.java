...
public class ArrayMethods {
	...
	public static void main(String[] args) {
	      ...
	      * */
		int[] arr = {9,6,3,4,3,8,9};
		int[] arr2 ={9,6,3,4,3,6,7};
		//print(getStats(arr));
		System.out.println(longestSharedSequence(arr,arr2));
	    }
	    ...
	    public static double[] getStats(double[] array){
	        ...
	        if(array.length%2 == 0){
	        	stats[3] = (array[((array.length)/2)] + array[((array.length)/2)-1] )/2;
	        		...
	        		stats[3] = array[((array.length)/2)-1];
	    ...
	    private static void swap(double[] array, int tempLowIndex, int i) {
	    ...
	    public static int longestConsecutiveSequence(int[] array1){
	    	...
	    	for(int i = 0;i<array1.length;i++){
	    		int index = i;
	    		while(index<array1.length-1&&array1[index]+1 == array1[index+1]){
	    	...
	    	for(int i = 0;i<array1.length; i++ ){
	    		for(int j =0; j<array2.length;j++){
	    			if(array1[i]==array2[j]){
	    				...
	    				int item2 = j;
	    				int count = 1;
	    				while(array1[item1++] == array2[item2++] && item2 < array2.length-1 && item1 < array1.length-1 ){
	    					count++;
	    					item1++;
	    					item2++;
	    				}
	    ...
	    public static int[] generateDistinctItemsList(int n){
	    	...
	    	for(int i = 0; i<nums.length;i++){
	    		...
	    		int newNum = (int) (((2*n)*rand)+1);
	    		while(isUsed(nums,newNum)){
	    			 rand = Math.random();
	    			newNum = (int) (((2*n)*rand)+1);
	    		}
	    		nums[i] = newNum;
	    	}
	        ...
	        return nums; 
	    	 ...
	    	 private static boolean isUsed(int[] arr,int key){
	    ...
	    public static void print(int[] ds){
